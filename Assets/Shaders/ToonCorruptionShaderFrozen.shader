Shader "Custom/ToonCorruptionShaderFrozen" {
	Properties 
    {
    	//Base Parameters
		...
		_OriginFrozenCore ("OriginFrozenCore", Vector) = (0.0,0.0,0.0,0.0)
		_DistanceFreezeDisappear ("DistanceFreezeDisappear", Float) = 1000
    ...
    Subshader 
    {
        ...
        Pass 
        {
            ...
            CGPROGRAM
				...
				float4 _OriginFrozenCore;
				float _DistanceFreezeDisappear;
				#endif
               ...
               v2f_surf vert_surf (appdata_full v) {
				...
				inline bool isFrozen(v2f_surf i){
					#if _CORR_ON
					float _FrozenWave = _OutlineSize / 5;
					float yPosition = distance(i.world,_OriginFrozenCore);
                   
                   //If between corrupted and nonCorrupted
                   if(yPosition<(_DistanceFreezeDisappear+_FrozenWave) && yPosition>(_DistanceFreezeDisappear)){
            			float magnitude = 1.0 - (((_DistanceFreezeDisappear+_FrozenWave) - yPosition)/_FrozenWave);
            		
						fixed4 dissolveColor = tex2D( _DissolveNoise, (i.world.xz*(_TileSize*2.0)));
						fixed highest = 0;
						if(dissolveColor.r>highest){highest = dissolveColor.r;}
						if(dissolveColor.g>highest){highest = dissolveColor.g;}
						if(dissolveColor.b>highest){highest = dissolveColor.b;}
	            	...
	            	}else if(yPosition>_DistanceFreezeDisappear){
	            		return true;
	            	}
	            	#endif
	            	return false;
				...
				inline fixed4 applyFrozen(v2f_surf i,float4 colorO){
               	 	#if _CORR_ON
	                   //If between corrupted and nonCorrupted
	                   if(isFrozen(i)){
	                   		return colorO;
